...
<ion-content class="nexus-content">
  <div class="content-pad">
    <div style="text-align: center; margin-bottom: 2rem;">
      ...
      <h1 class="page-title" style="margin-top: 0.5rem;">Historial de Resúmenes</h1>
    </div>
    ...
    @if (loading) {
      ...
      <div style="padding: 3rem 1rem; text-align: center; border: 1px dashed var(--nexus-border); border-radius: 12px;">
        ...
        <p style="color: var(--nexus-muted-2); margin: 0;">Aún no has generado resúmenes.</p>
      </div>
    } @else {
      <div style="display: flex; flex-direction: column; gap: 12px;">
        @for (item of items; track item.id) {
          <a class="nexus-card" [routerLink]="['/documents', item.document_id]" style="display: flex; align-items: center; padding: 1rem; text-decoration: none; border-radius: 12px;">
            <div style="width: 40px; height: 40px; border-radius: 8px; background: rgba(var(--ion-color-primary-rgb), 0.1); display: flex; align-items: center; justify-content: center; margin-right: 12px; color: var(--nexus-primary);">
              <ion-icon name="reader-outline" style="font-size: 1.5rem;"></ion-icon>
            </div>
            <div style="flex: 1; overflow: hidden;">
              <h4 style="margin: 0; font-size: 1rem; color: var(--nexus-text); font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ item.documents?.title || 'Documento sin título' }}</h4>
              <p style="margin: 4px 0 0 0; font-size: 0.85rem; color: var(--nexus-muted-2);">Resumen IA</p>
            </div>
...
export class AiSummariesPage implements OnInit {
  items: any[] = [];
  loading = true;
  ...
  async ngOnInit() {
    try {
      ...
      if (user) {
        this.items = await this.dashboardService.getSummariesHistory(user.id);
      }
    } catch (e) {
      console.error(e);
    } finally {
