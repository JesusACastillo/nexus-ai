...
<ion-content class="nexus-content">
  <div class="content-pad">
    <div style="text-align: center; margin-bottom: 2rem;">
      ...
      <h1 class="page-title" style="margin-top: 0.5rem;">Historial de Chats</h1>
    </div>
    ...
    @if (loading) {
      ...
      <div style="padding: 3rem 1rem; text-align: center; border: 1px dashed var(--nexus-border); border-radius: 12px;">
        ...
        <p style="color: var(--nexus-muted-2); margin: 0;">Aún no has tenido conversaciones AI.</p>
      </div>
    } @else {
      <div style="display: flex; flex-direction: column; gap: 12px;">
        @for (item of items; track item.id) {
          <a class="nexus-card" [routerLink]="['/chat/document', item.document_id]" style="display: flex; align-items: center; padding: 1rem; text-decoration: none; border-radius: 12px;">
            ...
            <div style="flex: 1; overflow: hidden;">
              <h4 style="margin: 0; font-size: 1rem; color: var(--nexus-text); font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ item.title || item.documents?.title || 'Chat sin título' }}</h4>
              <p style="margin: 4px 0 0 0; font-size: 0.85rem; color: var(--nexus-muted-2);">Última act. {{ item.updated_at | date:'shortTime' }}</p>
            </div>
...
export class AiChatHistoryPage implements OnInit {
  items: any[] = [];
  loading = true;
  ...
  async ngOnInit() {
    try {
      ...
      if (user) {
        this.items = await this.dashboardService.getChatConversationsHistory(user.id);
      }
    } catch (e) {
      console.error(e);
    } finally {
